    ...
    if "game_state" in msg.topic:
        ...
        if coins:
            ...
            if path:
                ...
                print("Moving player: ", player_name_2, " towards coin ", target_coin, " with move ", move_cmd)
                client.publish(f"games/{lobby_name}/{player_name_2}/move", move_cmd)
            # Randomize move (as backup)
            else:
                print("No path found, moving randomly.")
                bot_move = random.choice(moveset_list)
                client.publish(f"games/{lobby_name}/{player_name_2}/move", bot_move)
...
if __name__ == '__main__':
    ...
    client = paho.Client(callback_api_version=paho.CallbackAPIVersion.VERSION1, client_id="PlayerB2", userdata=None, protocol=paho.MQTTv5)
    ...
    client.subscribe(f'games/{lobby_name}/{player_name_2}/game_state')
